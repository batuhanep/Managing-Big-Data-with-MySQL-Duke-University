...
get_ipython().run_cell_magic('sql', '', 'SELECT SUM(ISNULL(exclude)) as nullexclude\nFROM dogs')
...
get_ipython().run_cell_magic('sql', '', "SELECT test_name, AVG(rating) as AVG_Rating, MIN(rating) as Min_Rating, MAX(Rating) as MAX_Rating\nFROM reviews\nWHERE test_name='Memory versus Pointing';")
...
get_ipython().run_cell_magic('sql', '', 'SELECT dog_guid, start_time, end_time, TIMESTAMPDIFF(minute, start_time, end_time) AS Duration\nFROM exam_answers\nLIMIT 200;')
...
get_ipython().run_cell_magic('sql', '', 'SELECT AVG(TIMESTAMPDIFF(minute, start_time, end_time)) AS Avg_Duration\nFROM exam_answers;')
...
get_ipython().run_cell_magic('sql', '', 'SELECT MIN(TIMESTAMPDIFF(minute, start_time, end_time)) AS Min_Duration,MAX(TIMESTAMPDIFF(minute, start_time, end_time)) AS Max_Duration\nFROM exam_answers;')
...
get_ipython().run_cell_magic('sql', '', 'SELECT COUNT(TIMESTAMPDIFF(minute, start_time, end_time)) AS Count_Negative_Duration\nFROM exam_answers\nWHERE TIMESTAMPDIFF(minute, start_time, end_time)<0;')
